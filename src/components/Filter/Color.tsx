...
const Color = ({ selected, setSelected }: SelectedProps) => {
    ...
    useEffect(() => {
        if (selected.length > 0) {
            // seçili renk varsa aralarına , koyup url e ekle
            params.set("color", selected.join(","));
            setParams(params);
        } else {
            // seçili renk yoksa url'de parametreyi kaldır
            params.delete("color");
            setParams(params);
        }
    }, [selected]);
                ...
                {colors.map((i) => {
                    ...
                    return (
                        <p
                            ...
                            key={i.id}
                            onClick={() => toggle(i.id)}
                            className={`py-[8px] px-[16px] text-center rounded-md  cursor-pointer transition hover:bg-zinc-400 text-transparent select-none ${found ? "ring-[4px]" : ""
                                }`}
                        >
                            .
                        </p>
