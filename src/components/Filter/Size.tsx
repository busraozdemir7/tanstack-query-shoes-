...
const Size = ({ selected, setSelected }: SelectedProps) => {
    ...
    useEffect(() => {
        if (selected.length > 0) {
            // seçili numaralar varsa aralarına , koyup url'e ekle
            params.set("size", selected.join(","));
            setParams(params);
        } else {
            // seçili numara yoksa url'de parametreyi kaldır
            params.delete("size");
            setParams(params);
        }
    }, [selected]);
                ...
                {numbers.map((num) => {
                    ...
                    return (
                        <p
                            key={num}
                            onClick={() => toggle(num)}
                            className={`py-[8px] px-[16px] lg:px-[0px] text-center rounded-md  cursor-pointer transition hover:bg-zinc-400 ${found ? "bg-black text-white" : "bg-white text-dark"
                                }`}
                        >
                            <span>{num}</span>
                        </p>
